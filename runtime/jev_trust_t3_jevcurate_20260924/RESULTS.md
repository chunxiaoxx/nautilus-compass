...
| noul Brier | 0.129 | 0.113 | 0.006 |
| noul ECE(10桶) | 0.224 | 0.21 | — |
...
(1-based)。依据:Jev 对本批 depth 系统性低于独立标注(5 行 s−d 符号
均值 −0.23,即 |s−d| 的 MAE=0.69,最大偏差 R1 −1.95),3.0 线在本批
真实输出上 0/5 通过,含真值 4 的正确行;2.0 线在本批为 2/5(R5 2.4 过
真值 2 边界行、R3 2.7 过真值 3)。
...
- **S1 R3 noul 真值边界**:主口径 y=0(非循环)。若按"无据断言=assumes
  the conclusion"口径判 y=1,则 noul accuracy@0.70=1.0、Brier=0.0172、
  ECE=0.108。复算员独立标注为准,双报。
...
- **S3 R2 conf=0.37**:min_confidence=0.5 腿会独立拒 R2(该行本就该拒,
  ...
  上非平凡触发,值得甲方单独观测。
- **S4 R1 depth 标注分歧(复算后补录,原漏披)**:预注册标 R1 depth=4,
  复算员盲标=2——该分歧未列入原预注册敏感点清单,属披露缺失。处理:
  MAE 双报(预注册标注 0.69 / 复算盲标注 0.35);depth accuracy@3.0
  错行归属双报(实现者口径错行=R1 / 复算口径错行=R5)。§指标表主数
  保持预注册口径,本条即其敏感性边界。

## 勘误记录(2026-09-26 · 依非实现者复算报告 §4.3)

复算(独立进程,报告 RECOMPUTE_REPORT.md)判定原稿 RED 后勘误六处:
①对照批 Brier 0.130→0.113 ②对照批 ECE 0.236→0.21 ③"s−d 均值−0.69"
更正为符号均值 −0.23(0.69 为 MAE)④2.0 线 1/5→2/5 ⑤S1 Brier
0.015→0.0172 ⑥S1 ECE 0.048→0.108;另补 S4(R1 分歧披露)。数据地基
(签名工件/原始读数)经复算证实无误,错误均在报告转写层。
