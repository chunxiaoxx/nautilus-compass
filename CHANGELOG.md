...
> 记忆不再只"存与召回",而是进入"提炼燃料→外部裁决→蒸馏变强"的进化循环。
> 依据 ASSET_MAP_v3 全量测绘(480 文件/7.7 万行/162 测试)如实编写。

### 引擎(生,实测)
- 语义 recall 复活:修复 Windows torch 长路径(WinError 206)导致的本地召回 100% 坏死。
- recall 直交付正文摘录(hit.body,消费方免二跳读文件)。
- 目标模式:GOAL SSOT 账本 + 每小时心跳执法(探活/自愈拉起/合约到期亮牌),首日立功。
- 云端容量三连根因修复(泄漏 sshd 834 核时/openclaw 双 systemd 互杀 32920 次/BGE 信号量泄漏),
  load 10-14→1.x;记忆池治理 68.8k→2967 文件,daemon 启动 11s(原小时级)。

### 进化循环(生,外部裁决)
- Gate B 真燃料管道 + Gold ×3(其中 2 真燃料 d3 r5/r15,commit 99184b6)。
- ③类部落事实 headroom 三级分类(通用常识零 headroom / 可推导不稳定 / 部落事实可靠)。
- recall 有用性对照实验:delta=1 首次外部证据,检索层 hit@3 = 9/9。
- 燃料入池机制:stop hook 自动提炼教训 + 周批 QC(control 先失败门)。

### 资产收编(冰山测绘,ASSET_MAP_v3)
- codex r15 王冠链(608 文件/10 万行)tag 保孤本;云生产孤本 mcp_http_server.py 入 repo;
  backup-0714 索引;26k 行论文/文档资产显性化。

### 诚实披露(未完成)
- 云主项目 pkl 重建中(回归门未过);外部用户为零;蒸馏判据(L4a)在跑未出;
  继承债见 GOAL SSOT(drift act-on rate 9.87% vs 目标 70%、Path B GTM 未启动、soul subscriber blocked)。
