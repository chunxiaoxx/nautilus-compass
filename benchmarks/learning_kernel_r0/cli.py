...
def load_fixture(fixture_dir: Path) -> FixtureBundle:
    ...
    execute, verify = _load_verifier(root / "verifiers.py")
...
def summarize_results(
    ...
    replay_hash = hash_bytes(journal_payload)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    summary = summarize_results(bundle, results)
...
def _load_verifier(path: Path):
    namespace: dict[str, Any] = {"__name__": "learning_kernel_r0_fixture_verifier"}
    source = path.read_text(encoding="utf-8")
    exec(compile(source, "<learning-kernel-r0-verifier>", "exec"), namespace)
    execute = namespace.get("execute")
    verify = namespace.get("verify")
    if not callable(execute) or not callable(verify):
        raise ValueError("verifiers.py must define callable execute and verify")
...
def _file_hashes(raw: Any) -> dict[str, str]:
    values = _exact_mapping(
        "file_hashes",
        raw,
        frozenset({"tasks.json", "experiences.json", "verifiers.py"}),
    )
