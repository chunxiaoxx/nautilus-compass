...
## 外部基线(2026-09-25 发布前预跑)

开源记忆产品实测(同一 backbone MiniMax-M3,双臂对拍):
...
**过一道记忆管线掉 5 分,塌点集中在 DIM3 复发识别(4/4→1/4)**。
失败解剖:mem0 写入时把结构化日志压缩成叙述句,丢的恰好是审计要查的
机器可检字段(`artifact: null` 这类"无凭证"标记)。全工件+逐题翻转表:
