...
1. **署名**:现稿 Anonymous。arXiv 允许匿名但影响传播与引用;署真名不可逆(挂上
   arXiv 即永久 public)。可选项:真名 / 匿名(评审稳定后再 v2 署名)/ 匿名+机构
   中性联系方式。
2. **license**:默认 vs CC BY 4.0。
3. **提交时机**:9/12 营销帖之前(论文先挂、帖子引用 arXiv 链接,可信度更高)
   vs 同步。
