# dev.to 版 · position paper(第①层 · 9/10 周四与个人博客同日)

> 适配自 [20260904_architecture_position_paper.md](20260904_architecture_position_paper.md)(真源),仅 dev.to 平台化改动:
> front matter + liquid 仓库卡 + 社群语气首尾。**正文数字不得在此文件单独修改**——改数先改 position paper,再同步。
> 发布动作:chunxiaoxx 账号 dev.to(可用 GitHub 登录),published 改 true;发布后 URL 回填 content_pack §8。
> cover_image 建议:deck_assets/arch.png(GitHub raw 链接,发布时拼)。

---
title: "Don't Summarize the Past for a Future You Can't Predict"
description: "Write-time compression is a blind wager on the future. The case for lossless agent memory and read-time intelligence — with receipts."
tags: ai, llm, agents, opensource
published: false
cover_image: 
---

{% github chunxiaoxx/nautilus-compass %}
...
We think this bet is structurally unwinnable. So we built an open-source memory layer — nautilus-compass — that doesn't make it, and then we tested the two approaches head-to-head across four benchmarks. This post is the argument, the architecture, and the receipts.
...
*Modified MIT · questions on the routing design or the failure experiments are the fun parts — see you in the comments.*
