...
| 中文圈三件(架构文中文版/知乎答/公众号技术文) | ② | 🟡 三份草稿在(zhihu_answer/v2ex_share/wechat_tech · 9/5 v1;🆕 9/6 备料终检:数字与定案逐项一致+pip 3.1.2 通道接入+知乎 arXiv 超前表述修正);9/15 周发布,等用户排期;position paper 中文版(知乎专栏长文)视第①层反响决定是否加做 |
| HN 提交文案(标题+首评) | ① | ✅ 见 §14 |
| 落地页 hero 刷新(75.4/81.6) | ④ | ✅ 9/5 实测已刷(big=75.4%·双口径·42.6→75.4 进步叙事·0.890 对比在位,零旧口径残留) |
...
- **"LME-V2 官方榜你们才 40 分,前十都 70+?"**:口径三差异先亮——预算 24k vs 官方 200k(1/8)、延迟 0.57s vs 26.9-108s(1/47 以下)、判分全题 LLM judge(更严)vs 官方程序化为主;这不是同一场考试。单位预算/单位时间的效率是我们当前可守的差异化;三池移植目标 55-60% 是公开的攻坚路径(ATTRIBUTION.md 有逐字段的坐标与差距来源)
