...
**判读**:①单用户/小并发(1-5)p50 ≤1.1s,体验良好;②**读吞吐天花板 ≈5 rps(全服务共享)**——并发加 20 倍吞吐只涨 1.7 倍,瓶颈在读路径(疑似单实例 CPU 嵌入/单进程);③写路径扩展性佳(28.6 rps @20 并发);④韧性 OK,无 5xx 雪崩。**注意:本次压测的 probe 用户是空记忆空间(下限近似),真实用户大库检索单请求更重。**

**发布日含义**:预期引流(signup 数十到百级、每个用户低频试 MCP)下读并发瞬时 5-10 → p95 5-8s,**慢但不死**,open beta 可接受。降压预案(任选,均需拍板):a) 服务器 uvicorn 多 worker 重启(最直接);b) 发布帖注明 open beta 高峰慢;c) 不动,观察真实流量再说。
