...
def test_current_policy_gate_evidence_allows_guarded_and_blocks_raw():
    ...
    assert gate["source"]["profile_dir"] == ".cache/bench-profile-20260721-040249-(default in daemon.py)"
    assert gate["source"]["commit"] == "9632954"
    assert guarded["status"] == "accept"
...
def test_release_workflow_runs_recall_policy_preflight():
    ...
    assert "--target-policy guarded" in text
