...
> 🔴 **8/30 正本清源裁决(用户触发)**:本节及下文历史增量中所有"蒸馏线收棺归档""权重翼收棺/终审""北极星降级/唯一引擎=自治翼"表述系平台越权升格(业务暂停→技术死亡、配方负结果→方向终审),**作废**。正本=用户原话:中心环两维度缺一不可;高质量数据集→训练小模型是主线;配方负结果=排除非终点。全文与修正清单:`docs/RECTIFICATION_20260830_正本清源.md`。分工正本(8/30 用户口述):**平台=数据管道 · V5=超级agent+训练执行 · compass=不断出题 · flywheel=数据飞轮业务**。防复发条款:**方向生杀=用户专属,框级只能写"X 配方在 Y 任务 NEGATIVE"**。

> 🔴 **8/31 AI 原生组织运行协议 v1 生效(用户拍板)**:决策权三级(L1 自决报告/L2 自决追认/L3 升级)+默认裁决 24h 条款+各框 daemon 化认领+用户接口=日报。**§2 预算已锁定(用户 8/31 拍板):单笔≤¥30/单框日≤¥60/单框月≤¥500/全组织月≤¥1500 熔断,全部 L2 花销强制进日报。** 全文 `docs/AI_NATIVE_ORG_PROTOCOL_v1.md`。用户日常参与收敛为:读日报→拍 L3→否决 L2(可选)。
...
- **RSI 形态=组织级自我迭代**(8/30 修正:权重翼未被"收棺"——g2b1 修复类配方六连负是配方排除,协议域 v3 PROVEN 证明权重注入有边界有路径;RSI 组织层与权重层两翼并行,缺一不可)。
- **路径=三台阶不变**(台阶一连环→台阶二目标自长→台阶三外部真值裁决),工程含义=把"人肉纪律"(读 SSOT/同步副本/写交接/手动对账)逐条降级为平台自动件。
- **北极星(8/30 修正,恢复两翼完整)**:中心环两维度缺一不可——(a) 模型权重变强(蒸馏:高质量数据集→训练小模型,配方按负结果迭代) (b) agent 群体自治变强。FDE 出题训练**业务**暂停(经营事实,8/28 用户原话),数据管道(采集/标注/清洗/评测/验证)是权重翼的第一环并向数据飞轮(具身)业务吃狗粮——**管道是能力,蒸馏是目的,两者不是替代关系**。
...
> **🔴 8/30 十九次增量(V5 框 · d6 定案 NEGATIVE · 权重翼终审)**:SFT+RL 涡轮四臂评测判读完成(base/SFT/DPO/SFT+DPO × train8/held-out20,产物 `vtf/_d6_eval4/` 176 条 + 双源判分一致)。**train t8(k=3):base 1/8 · SFT 2/8 · DPO 0/8 · 组合 1/8;held-out 四臂全 0。三态判定 = NEGATIVE**(预注册字面:四臂域内全 ≤55%;PROVEN ≥65%+泛化>0 双落空;消融新证 = SFT 底座对 DPO 无增压)。**修复类蒸馏全线关闭(SFT+DPO+GRPO),蒸馏仅存协议格式域(v3 PROVEN);六次尝试矩阵闭合(v1负/v2v3协议域正/v4负/d4负/d5负/d6负),与 8/27"权重翼收棺"互证**。再解冻条件=外部方法突破或协议域商用收入反哺(预注册盖棺条款)。资产:568 轨迹+GLM 补采转交付燃料;预算未超 ¥35 帽。口径偏差如实记录(k=3 执行 vs k=8 预注册,偏保守方向);可选加固 ¥7 补跑默认不做。GPU 651057 c1 实测仍活。已并入 canonical(平台 8/30 执行;trace v5-d6-negative-20260830)。定案全文:`docs/plans/2026-08-27-dpo-round-preregistration.md` 结果节。

> **🔴 8/29 十八次增量(平台 · 分支正名专场完成)**:云主仓从 soul-audit-increment1+61dirty 正式切回 **soul-distill-deploy 干净树**——61 文件六组判定落地(kairos2 改造 7 件 commit 保留/executor evidence ~30 件入库/换行噪声归位/备份入 gitignore),/home/ubuntu/distill-deploy 老 worktree(613 dirty)清除,`pull --rebase` 正规部署流程回归(终结 cherry-pick 绕行)。意外收获:牙齿生产首咬抓到 check_mailbox mapper 链 bug(已修已部署);3 条 telegram 提前催办发出(v5/compass/flywheel)。云 verified 526 持续涨。判定清单:docs/plans/2026-08-29-branch-rename-audit.md。

> **🔴 8/29 十七次增量(凌晨 · workbuddy 首调里程碑+org OS P0/P0.5)**:①**workbuddy 首调成功(用户实测)**——`~/.workbuddy/mcp.json` 落地(cmp_workbuddy token+9877 隧道+ops 桥,烟测握手 10s),平台对外 MCP 第一个外部客户端真实消费;8/22 挂账清零;三框 workbuddy 线同源(创投 296 行插件战略/compass P2 提审/平台底座)。②**org OS P0+P0.5 落地**(commits 5654eb801/7ffcaae8c):mailbox 端点(幂等/deadline 必带/ack)+supervisor 牙齿(due<12h 未 ack→telegram,notified_at 去重),TDD 11 green;云部署待分支正名打包。③skills 二轮 52→35(工作流套件被模型替代判据),全局 rules 22→10,phase3/CLAUDE.md 化石重写。④loop 30min 重启(用户指示,13/43 分避开整点);V5 三台阶认领(台阶一 executor 曲线+自愈三件套,DPO 随缘收尾);创投框出 WorkBuddy 插件战略 296 行。⑤催办双通道(仓根+compass 云 obs)40 分钟见效=mailbox 前身的首次验证。
