...
\subsection{End-to-end with full retrieval stack (n=100)}
...
on the same topic 01 with DeepSeek-V4-flash as both answerer and judge
(matching our LongMemEval pipeline · 14.9 minutes wall-clock on a
single T4):
...
+ Mem0                              & 37.09 \\
+ Zep                               & 39.97 \\
\textbf{compass (BGE-m3+rerank+V4-flash)}  & \textbf{40.00} \\
+ MemOS                             & 42.55 \\
+ EverCore                          & not reported \\
\bottomrule
\end{tabular}
\caption{compass on EverMemBench-Dynamic topic 01 (n=100) alongside
the paper Table 4 baselines. compass falls between Zep (39.97) and
MemOS (42.55), filling the EverCore gap with an open-source,
self-hosted alternative. Recall@20 was 91.0\% (BM25-only baseline:
38.1\%); the 51-point recall improvement explains where the dense
retriever helps. The remaining gap to MemOS is likely due to (a)
different LLM (V4-flash vs GPT-4.1-mini), (b) topic 01 only vs
full 5-topic average, and (c) no Profile-Understanding subtask
yet. Caveat: n=100 has $\pm$10pt 95\% CI · we do not claim a
ranking until full 5-topic 2400-QA run completes.}
