...
on all 5 topics with DeepSeek-V4-pro think-high as the answerer and
V4-flash as the judge, with multi-angle query rewriting (3 angles,
union-deduplicated) and day-bucket diversification (max 2 sessions per
date). 5.58 hours wall-clock on a single T4 $\cdot$ \$25 LLM API:

\begin{table}[h]
\centering
\begin{tabular}{lrr}
\toprule
Topic & accuracy & recall@30 \\
\midrule
01 & 44.0\% & 97.0\% \\
02 & 46.0\% & 95.0\% \\
03 & 42.0\% & 98.0\% \\
...
05 & 45.0\% & 99.0\% \\
\midrule
\textbf{All (n=500)} & \textbf{44.4\%} & \textbf{97.6\%} \\
\bottomrule
\end{tabular}
\caption{compass per-topic results on EverMemBench-Dynamic (v2 driver,
2026-05-07). Cross-topic CV $=$ 4\% (consistent $\cdot$ low variance).}
...
+ Zep                               & 39.97 \\
+ MemOS                             & 42.55 \\
\textbf{compass (BGE-m3+rerank+rewrite+day-bucket+V4-pro $\cdot$ n=500)} & \textbf{44.40} \\
+ EverCore                          & not reported \\
\bottomrule
\end{tabular}
\caption{compass on EverMemBench-Dynamic full 500 (5 topics $\cdot$
n=100/topic) alongside the paper Table 4 baselines. compass at 44.4\%
exceeds MemOS (42.55) by 1.85 points and places above all four
reported baselines, filling the EverCore gap with an open-source,
self-hosted alternative. Recall@30 was 97.6\% (BM25-only baseline:
38.1\%); the dense retriever + cross-encoder + multi-angle rewrite
stack closes nearly the entire retrieval gap. Per-topic CV is 4\% $\cdot$
cross-topic stability is high.}
