...
(matching our LongMemEval pipeline · 76 minutes wall-clock on a single T4 · ~\$1.50):
...
Cross-topic CV $=$ 6\% (consistent · low variance).}
...
\begin{tabular}{lr}
\toprule
System (paper Table 4 · GPT-4.1-mini answerer) & Avg \\
\midrule
Full Context (1M tokens, no memory) & 37.44 \\
+ MemoBase                          & 34.27 \\
+ Mem0                              & 37.09 \\
+ Zep                               & 39.97 \\
\textbf{compass (BGE-m3+rerank+V4-flash · n=500)} & \textbf{41.00} \\
+ MemOS                             & 42.55 \\
...
is likely due to (a) different LLM (V4-flash vs GPT-4.1-mini ·
...
Style/Skill/Role · we measured Fine-Grained Recall + Memory Awareness only).
Per-topic CV is 6\% · cross-topic stability is high.}
\label{tab:em-full-stack}
\end{table}

\subsection{Bug archeology · how we got here}
...
out of retrieved context · LLM correctly returned UNKNOWN. Fix:
\texttt{[:1500]}.
\item DeepSeek V4-flash defaults to \texttt{thinking} mode and
\texttt{reasoning\_mode="non-think"} did not disable it · 50--200
...
yielded an empty \texttt{content} string · ``\texttt{CORRECT} in
\texttt{""}'' was always False · the entire run was scored 0\%.
