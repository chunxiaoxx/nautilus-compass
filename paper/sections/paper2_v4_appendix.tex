% Paper 2 	extperiodcentered{}  Appendix C 	extperiodcentered{}  DeepSeek V4-pro: a tied-accuracy negative result
% Status: 2026-05-07 	extperiodcentered{}  full 500 LANDED 	extperiodcentered{}  56.4% 	extperiodcentered{}  -0.2 vs v0.8 	extperiodcentered{}  CLOSED

\section{DeepSeek V4-pro: a tied-accuracy negative result}
...
identical (m3-rerank 	extperiodcentered{}  query rewriting 	extperiodcentered{}  type-aware prompts).

\subsection{Setup}

\begin{itemize}
\item Subject + Judge: DeepSeek V4-pro (subject) 	extperiodcentered{}  V4-flash (judge 	extperiodcentered{} 
  cross-family avoided since same vendor 	extperiodcentered{}  documented limitation).
\item Reasoning mode: \texttt{think-high}
\item Context: full 1M tokens 	extperiodcentered{}  no truncation needed for LongMemEval-S
  50K haystacks.
...
\item Cost: \$1.40 USD for 50 questions (vs \$0.15 USD for v0.8 50q
  on Volc Ark coding plan 	extperiodcentered{}  9× more expensive).
\end{itemize}

\subsection{Sample 48 results (n=48 stratified 	extperiodcentered{}  8 per type)}
...
\begin{tabular}{lrrr}
\toprule
\textbf{Type} & \textbf{V0.8 (V3.2)} & \textbf{V4-pro think-high} & \textbf{\(\Delta\)} \\
\midrule
knowledge-update         & 57.7\% & \textbf{75.0\%} (6/8) & \textbf{+17.3} $\star$ \\
single-session-assistant & 83.9\% & 75.0\% (6/8) & \(-8.9\) \\
single-session-user      & 57.1\% & \textbf{75.0\%} (6/8) & \textbf{+17.9} $\star$ \\
multi-session            & 54.9\% & 62.5\% (5/8) & +7.6 \\
single-session-preference& 53.3\% & 50.0\% (4/8) & \(-3.3\) \\
temporal-reasoning       & 46.6\% & \textbf{25.0\%} (2/8) & \textbf{\(-21.6\)} $\dagger$ \\
\midrule
\textbf{Overall (sample)} & 56.2\% & \textbf{60.4\%} & +4.2 \\
\bottomrule
\end{tabular}
\caption{DeepSeek V4-pro \texttt{think-high} mode 	extperiodcentered{}  sample 48. Three
question types (ku 	extperiodcentered{}  ssa 	extperiodcentered{}  ssu) reach 75\% accuracy, demonstrating that
...
\item \textbf{Cost vs gain trade-off.} V4-pro think-high at \$0.028/q
  ...
  overall gain projection means roughly \$0.005 per accuracy point 	extperiodcentered{} 
...
\begin{itemize}
\item Only 35/50 questions completed at submission time 	extperiodcentered{}  final
  numbers may shift ±3 pts.
\item V4-pro + V4-flash judge is \emph{not} a true cross-family check 	extperiodcentered{} 
...
\item think-max mode untested 	extperiodcentered{}  may push accuracy higher at further
  cost increase.
\end{itemize}
...
\texttt{think-high} (no \texttt{max\_tokens} cap 	extperiodcentered{}  vanilla pipeline 	extperiodcentered{} 
...
\begin{tabular}{lrrrr}
\toprule
\textbf{Type} & \textbf{v0.8 (V3.2 n=500)} & \textbf{V4 sample (n=48)} & \textbf{V4 full (n=500)} & \textbf{\(\Delta\) vs v0.8} \\
\midrule
...
hours on a single T4 (CPU bge-m3 	extperiodcentered{}  GPU contention with cross-judge
runs).}
\label{tab:v4-full500}
\end{table}

\subsection{Verdict and sample-size lesson}
\label{app:v4-verdict}

V4-pro \texttt{think-high} is essentially tied with V3.2
(\(-0.2\) pts overall 	extperiodcentered{}  within noise). Per-type, V4 wins clearly on
ssa (\(+3.6\)) and ssp (\(+13.4\)) but loses on multi-session
(\(-2.3\)) and temporal-reasoning (\(-3.0\)). The headline number for
this paper remains v0.8 V3.2 56.6\%, and V4-pro is preserved here as
a closed negative result, not a future milestone.
...
selective routing (V4 for ssa/ssp 	extperiodcentered{}  V3.2 for ms/temporal) is the only
path to a positive economic case 	extperiodcentered{}  neither is in scope for v1.0 paper.

% [Removed 2026-05-07] The "Decision (2026-05-05): held full 500"
% subsection is obsolete: the full 500 has since been run (see
% Section~\ref{app:v4-verdict}). The sample-48 hold rationale and the
% follow-up max_tokens / think-max enumerations belong in lab notes,
% not in the camera-ready appendix.
